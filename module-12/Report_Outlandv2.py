...
def equipment_aging_report():
    ...
    query = """
    SELECT EquipmentID, Type, TIMESTAMPDIFF(MONTH, PurchaseDate, CURDATE()) AS MonthsSincePurchase
    FROM Equipment;
    """
    ...
    plt.barh(df_equipment_aging['Type'], df_equipment_aging['MonthsSincePurchase'], color='salmon')
    ...
    for row in result:
        print(f"Equipment ID: {row[0]}, Type: {row[1]}, Months Since Purchase: {row[2]}")
